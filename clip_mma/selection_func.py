...
def visualize_latent_space(modelname_dict, category_label, model_feature,save_dir):
    """
...
def visualize_similarity_in_latent_space(model_feature,target_feature, sim, save_dir):
    """
...
def display_histogram(modelname_dict,category_label, sim, save_dir):
    """
...
def measure_similarity(pil_img, config, save_dir, auto_crop=True):
    ...
    model_feature = mapper.transform(model_embeddings)
    target_feature = mapper.transform(target_vector)

    #The results are shown and saved in the following.
    visualize_latent_space(modelname_dict, category_label, model_feature,save_dir)
    visualize_similarity_in_latent_space(model_feature,target_feature, sim, save_dir)
    display_highrank_images(pil_img, image_path_list, sim, save_dir,DISPLAY_IMAGE_NUMBER)
    display_histogram(modelname_dict, category_label, sim,save_dir)
